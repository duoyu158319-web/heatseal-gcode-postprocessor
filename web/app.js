import { generateGcode, getReplayCandidates, md5Text, parseGcode, parseObjectNames, summarizeLayer } from "./core.mjs";
...
function renderTimeline() {
  ...
  $("#timeline").innerHTML = rows.map((r) => `<button class="layer-tick ${r.paused ? "has-pause" : ""}" data-layer="${r.number}" title="第 ${r.number} 层 · Z${r.z ?? "?"} · ${r.walls} 条墙轨迹"><span style="height:${Math.max(10, r.walls / max * 62)}%"></span>${r.number === 1 || r.number === state.parsed.totalLayers || r.paused ? `<small>${r.number}</small>` : ""}</button>`).join("");
}
...
function draw(cfg) {
  requestAnimationFrame(() => {
    ...
    const b = selected.reduce((a, t) => ({ minX: Math.min(a.minX, t.bounds.minX), maxX: Math.max(a.maxX, t.bounds.maxX), minY: Math.min(a.minY, t.bounds.minY), maxY: Math.max(a.maxY, t.bounds.maxY) }), { minX: Infinity, maxX: -Infinity, minY: Infinity, maxY: -Infinity });
    ...
    selected.forEach((t, i) => {
      ctx.strokeStyle = ["#ef6a3a", "#176b51", "#b98b2f", "#445e97"][i % 4]; ctx.lineWidth = i ? 1.7 : 2.4; ctx.beginPath(); ctx.moveTo(p(t.start).x, p(t.start).y);
      let current = { ...t.start };
      t.commands.forEach((command) => { const x = command.match(/(?:^|\s)X([-+.\d]+)/i)?.[1], y = command.match(/(?:^|\s)Y([-+.\d]+)/i)?.[1]; if (x !== undefined) current.x = Number(x); if (y !== undefined) current.y = Number(y); if (x !== undefined || y !== undefined) ctx.lineTo(p(current).x, p(current).y); });
      ctx.stroke();
    });
  });
}

const circleName = (index) => ["第一圈", "第二圈", "第三圈", "第四圈"][index] ?? `第 ${index + 1} 圈`;
...
const speedControl = (cfg, track, index) => {
  const factor = factorAt(cfg, index), offset = offsetAt(cfg, index), percent = Math.round(factor * 100), sourceSpeed = mmPerSecond(track.originalFeed), outputSpeed = sourceSpeed * factor;
  const offsetNote = offset === 0 ? "保持原始轨迹，不进行外扩" : `轨迹沿轮廓外侧等距移动 ${offset.toFixed(1)} mm`;
  return `<div class="loop-speed" data-loop="${index}"><div class="field-label speed-label"><span>${circleName(index)}速度</span><output>${factor.toFixed(1)} × · ${percent}%</output></div><div class="speed-control"><input class="speed-slider" type="range" min="0" max="1" step="0.1" value="${factor}" data-field="loopSpeedFactor" data-loop-index="${index}" aria-label="${circleName(index)}相对原路径速度"><div class="speed-scale">${speedScale(factor)}</div><p class="speed-note ${factor === 0 ? "is-error" : ""}">${factor === 0 ? "速度为 0 无法复走，请至少选择 0.1。" : `原路径 ${sourceSpeed.toFixed(1)} mm/s，复走 ${outputSpeed.toFixed(1)} mm/s`}</p></div><div class="field-label speed-label offset-label"><span>${circleName(index)}向外偏移</span><output>${offset.toFixed(1)} mm</output></div><div class="offset-control"><div class="offset-options" role="radiogroup" aria-label="${circleName(index)}向外偏移距离">${offsetOptions(offset, index, cfg.id)}</div><p class="offset-note">${offsetNote}</p></div></div>`;
};
...
function renderConfigs() {
  configs.innerHTML = state.configs.map((cfg, i) => {
    ...
    const speed = selected.length ? selected.map((t, index) => `第${index + 1}圈 ${mmPerSecond(t.originalFeed * factorAt(cfg, index)).toFixed(1)} mm/s`).join(" / ") : "—";
    ...
    return `<article class="config-card" data-config="${cfg.id}">
      ...
      <section class="path-preview"><div class="preview-head"><span>轨迹预览</span><b>${selected.length}/${maxCircles} 圈</b></div><canvas data-canvas="${cfg.id}"></canvas><div class="path-stats"><span>复走 Z <b>${sourceLayer?.z !== undefined ? (sourceLayer.z + .01).toFixed(2) : "—"}</b></span><span>输出速度 <b>${speed}</b></span></div></section></div>
...
configs.oninput = (e) => {
  ...
  card.querySelector(".path-stats span:last-child b").textContent = selected.length ? selected.map((item, index) => `第${index + 1}圈 ${mmPerSecond(item.originalFeed * factorAt(cfg, index)).toFixed(1)} mm/s`).join(" / ") : "—";
};
