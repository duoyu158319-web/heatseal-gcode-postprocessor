...
test("supports per-pause heat-seal pressure depth in 0.01 mm steps", () => {
  const config = configFor(fixture); config.pressDepth = .17;
  const block = heatSealBlock(generateGcode(fixture, [config]).text);
  assert.match(block, /at Z0.13; press depth 0.17mm/);
  ...
  const tooLarge = configFor(fixture); tooLarge.pressDepth = .21;
  assert.throws(() => generateGcode(fixture, [tooLarge]), /0.00–0.20 mm/);
