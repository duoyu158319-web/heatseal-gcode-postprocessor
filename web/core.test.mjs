...
import { analyzeLayerGeometry, classifyLayerRegions, generateGcode, getReplayCandidates, md5Text, offsetTrack, parseGcode } from "./core.mjs";
...
test("separates a merged model into skeleton and outer-ring regions", () => {
...
; Z_HEIGHT: 0.4`);
  ...
  assert.equal(regions.ringDetected, true);
  assert.equal(regions.outerRingTracks.length, 2);
  assert.equal(regions.skeletonWallTracks.length, 1);
  assert.deepEqual(candidates.tracks.map((track) => track.region), ["skeleton", "outer-ring", "outer-ring"]);
  assert.equal(candidates.tracks.at(-1).isOutermost, true);
  assert.equal(getReplayCandidates(parsed, 3, [], []).tracks.length, 0);
  const output = generateGcode(threeLoops, [{ layerNumber: 3, replay: true, circles: 3, outerRepeat: 2, speedFactors: [.1], offsetDistances: [0] }]).text;
  const block = output.match(/; HEATSEAL_POSTPROCESS_START[\s\S]*?; HEATSEAL_POSTPROCESS_END/)?.[0] ?? "";
  assert.equal(block.match(/; Replay loop/g)?.length, 4);
  assert.match(block, /region outer-ring; pass 1\/2/);
  assert.match(block, /region outer-ring; pass 2\/2/);
});
